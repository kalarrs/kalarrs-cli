...
module.exports = {
    ...
    async checkForWorkspaceDependencies(srcPath) {
        ...
        if (hasAllPackages) logUtil.success(`Default node packages found in workspace package.json`);
        else logUtil.success(`Default node packages are not all present in workspace package.json`);

        return hasAllPackages;
    },

    async checkForWorkspaceTypeScriptDependencies(srcPath) {
        const packageNames = [
            'typescript',
        ];
        const packageChecks = [];
        ...
        if (hasAllPackages) logUtil.success(`Typescript node packages found in workspace package.json`);
        else logUtil.success(`Typescript node packages are not all present in workspace package.json`);
