...
module.exports = {
    async checkForInit(srcPath) {
        const {stdout} = await execAsync('git rev-parse --is-inside-work-tree').catch(err => ({stdout: 'false'}));
        if (stdout.trim() !== 'true') {
            logUtil.error('git rev-parse --is-inside-work-tree returned error');
    ...
    async checkIfWorkspaceFilesIgnored(srcPath) {
        ...
        const filesChecks = await Promise.all(patterns.map(async (pattern) => {
            if (!await this.checkIfIgnored({srcPath, pattern})) return await this.addToIgnore({srcPath, pattern});
            return true;
        }));
        const hasAllFiles = filesChecks.every(i => i === true);
    ...
    async useRsaKeyWithGitHub(fileName) {
        ...
        let hasExistingRsaKey = await existsAsync(`${os.homedir()}/.ssh/${fileName}.pub`);
        ...
        if (!gitHubResponse.id && gitHubResponse.message) {
            ...
            switch (gitHubResponse.message.toLowerCase()) {
                ...
                case 'bad credentials':
                    throw new Error('Bad Username/Password for GitHub');

                default:
                    throw new Error('Could not add RSA key to GitHub. You will have to do that manually https://help.github.com/articles/adding-a-new-ssh-key-to-your-github-account');
            }
        }
    },

    async cloneWorkspaceTypescriptLibs(srcPath) {
        if (!await existsAsync(path.join(srcPath, 'libs'))) {
            await execAsync('git clone --depth=1 https://github.com/kalarrs/serverless-template-typescript', {cwd: srcPath});
            await execAsync(`${process.platform === 'win32' ? 'move' : 'mv'} serverless-template-typescript/aws/libs/`, {cwd: srcPath});
            if (process.platform === 'win32') await execAsync('rd /s /q serverless-template-typescript', {cwd: srcPath});
            else await execAsync('rm -rf serverless-template-typescript', {cwd: srcPath});
        }
    }
};
