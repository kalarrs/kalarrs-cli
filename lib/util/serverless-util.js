...
const programUtil = require('./programs-util');
...
module.exports = {
    async checkForWorkspaceUserYaml(workspaceRoot) {
        ...
        logUtil.warn('You do not have a ' + serverlessUserFileName + ' file.');
        ...
        let {user} = await
            prompt({
                ...
                validate: (input) => {
                    if (!(input && input.length)) return 'You must supply a username.';
                    return true;
                }
            });
    ...
    async checkForWorkspaceDevEnvYaml(workspaceRoot) {
        ...
        logUtil.warn(`You do not have your .env/${serverlessDevEnvFileName} file configured.`);
    ...
    async checkForWorkspaceServerlessYaml(workspaceRoot) {
        ...
        logUtil.warn('You do not a ' + serverlessConfigFileName + ' file.');
        ...
        const hasProfile = await programUtil.checkForAwsProfile(true, profile);

        data.provider.profile = hasProfile ? profile : 'default';
