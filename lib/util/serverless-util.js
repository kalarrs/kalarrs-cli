...
module.exports = {
    ...
    async create(tempalteUrl, projectPath) {
        if (!(tempalteUrl && tempalteUrl.length)) throw new Error('You must provide a template url');
        if (!(projectPath && projectPath.length)) throw new Error('You must provide a project path');
        await execAsync(`sls create --template-url=${tempalteUrl} --path projectPath`)
    }
};